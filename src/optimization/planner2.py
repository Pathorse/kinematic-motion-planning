...
from pydrake.all import (MathematicalProgram, SnoptSolver, IpoptSolver, eq, ge, le)
...
def set_initial_and_terminal_position(prog, robot, q_start, q_goal, decision_variables):
    ...
    # Initial states
    x_0 = robot.center_of_mass_in_M(q_start)
    r_fl_0, r_fr_0, r_rl_0, r_rr_0 = robot.forward_2D_kinematics(q_start)

    # Enforce initial states
    prog.AddLinearConstraint(eq(q[0], q_start))
    prog.AddLinearConstraint(eq(x[0], x_0))
    prog.AddLinearConstraint(eq(r_fl[0,:2], r_fl_0))
    prog.AddLinearConstraint(eq(r_fr[0,:2], r_fr_0))
    prog.AddLinearConstraint(eq(r_rl[0,:2], r_rl_0))
    prog.AddLinearConstraint(eq(r_rr[0,:2], r_rr_0))
...
# ----------------------------------------------------
def set_initial_guess(prog, robot, q_start, desired_net_distance, desired_turning_angle, decision_variables, time_interval, time_steps):

    # Unpack state and input
    q, x, r_fl, r_fr, r_rl, r_rr = decision_variables[:6]

    # Interpolate initial guess
    x_guess = linear_interpolation(q_start[:3], q_start[:3] + desired_net_distance, time_steps)

    q_b_pos_guess = linear_interpolation(q_start[:3], q_start[:3] + desired_net_distance, time_steps)
    q_b_ori_guess = np.zeros((time_steps, 3))
    q_b_guess = np.concatenate((q_b_pos_guess, q_b_ori_guess), axis=1)

    q_r_guess = np.array([q_start[6:]] * time_steps)

    rxy_fl_guess, rxy_fr_guess, rxy_rl_guess, rxy_rr_guess = footpoint_xy_interpolation(robot, q_start, desired_net_distance, time_steps)

    # Set initial guess
    prog.SetInitialGuess(x[:], x_guess)
    prog.SetInitialGuess(q[:,:6], q_b_guess)
    prog.SetInitialGuess(q[:,6:] , q_r_guess)
    prog.SetInitialGuess(r_fl[:,:2], rxy_fl_guess)
    prog.SetInitialGuess(r_fr[:,:2], rxy_fr_guess)
    prog.SetInitialGuess(r_rl[:,:2], rxy_rl_guess)
    prog.SetInitialGuess(r_rr[:,:2], rxy_rr_guess)
...
def set_state_constraints(prog, robot, decision_variables, time_steps):
    ...
    # Base bounds
    q_b_lb = np.array([- np.Inf, - np.Inf, - np.Inf, - np.pi, - np.pi, - np.pi])
    q_b_ub = np.array([np.Inf, np.Inf, np.Inf, np.pi, np.pi, np.pi] )

    # Joint bounds
    q_r_lb = np.array([- np.pi] * (robot.n_q - 6))
    q_r_ub = np.array([np.pi] * (robot.n_q - 6))

    # Generalized coordinate bounds
    q_lb = np.concatenate((q_b_lb, q_r_lb))
    q_ub = np.concatenate((q_b_ub, q_r_ub))
    ...
    # Footposition bounds
    r_lb = np.array([- np.Inf] * 2)
    r_ub = np.array([np.Inf] * 2)
    ...
    for t in range(0, time_steps):
        ...
        prog.AddLinearConstraint(r_fl[t,:2], lb=r_lb, ub=r_ub)
        prog.AddLinearConstraint(r_fr[t,:2], lb=r_lb, ub=r_ub)
        prog.AddLinearConstraint(r_rl[t,:2], lb=r_lb, ub=r_ub)
        prog.AddLinearConstraint(r_rr[t,:2], lb=r_lb, ub=r_ub)
...
# ----------------------------------------------------
# Set the footpoints height profile constraint
# ----------------------------------------------------
def set_height_profile(prog, robot, decision_variables, time_steps):
    ...
    # Get height profile
    rz_fl, rz_fr, rz_rl, rz_rr = robot.height_profile(time_steps)

    # Add height profile constraint
    for t in range(time_steps):

        prog.AddLinearConstraint(r_fl[t,2] - rz_fl[t] == 0)
        prog.AddLinearConstraint(r_fr[t,2] - rz_fr[t] == 0)
        prog.AddLinearConstraint(r_rl[t,2] - rz_rl[t] == 0)
        prog.AddLinearConstraint(r_rr[t,2] - rz_rr[t] == 0)
...
# ----------------------------------------------------
def set_support_polygon(prog, robot, decision_variables, contact_flags, time_interval, time_steps):
    ...
    for t in range(0, time_steps):
        ...
        center_of_pressure = robot.center_of_pressure_2D(x[t], x_ddot)

        support_polygon = c_fl[t] * w_fl[t,0] * r_fl[t,:2] + c_fr[t] * w_fr[t,0] * r_fr[t,:2] + \
                          c_rl[t] * w_rl[t,0] * r_rl[t,:2] + c_rr[t] * w_rr[t,0] * r_rr[t,:2]
        
        prog.AddConstraint(eq(support_polygon, center_of_pressure))
...
def set_periodic_motion(prog, decision_variables, time_steps):
    ...
    prog.AddLinearConstraint(eq(q[0, 6:], q[-1, 6:]))
...
def set_walking_and_turning_speed(prog, decision_variables, desired_net_distance, desired_turning_angle, time_steps):
    ...
    #prog.AddLinearConstraint(eq(x[-1] - x[0], desired_net_distance))
    prog.AddLinearConstraint(eq(q[-1,:3] - q[0,:3], desired_net_distance))

    # Enforce turning angle
    turning_angle_residual = q[-1, 5] - q[0, 5] - desired_turning_angle

    #prog.AddLinearConstraint(turning_angle_residual == 0)
...
# ----------------------------------------------------
def add_smoothness_cost(prog, decision_variables, time_interval, time_steps):
    ...
    for t in range(1, time_steps - 1):
        ...
        prog.AddQuadraticCost(10**(-6) * ((q[t - 1] - 2 * q[t] + q[t + 1]) / time_interval**2).dot((q[t - 1] - 2 * q[t] + q[t + 1]) / time_interval**2))
        #prog.AddQuadraticCost(((q[t - 1, 6:] - 2 * q[t, 6:] + q[t + 1, 6:]) / time_interval**2).dot((q[t - 1, 6:] - 2 * q[t, 6:] + q[t + 1, 6:]) / time_interval**2))
...
def run_NLP(robot, q_start, weight_lb, omega_lb, omega_ub, desired_net_distance, desired_turning_angle, \
    ...
    # Set initial guess
    set_initial_guess(prog, robot, q_start, desired_net_distance, desired_turning_angle, decision_variables, time_interval, time_steps)

    # State constraints
    set_state_constraints(prog, robot, decision_variables, time_steps)

    # Set height profile
    set_height_profile(prog, robot, decision_variables, time_steps)

    # Forward kinematic constraint
    set_kinematics(prog, robot, decision_variables, time_steps)

    # COP within support polygon constraint
    set_support_polygon(prog, robot, decision_variables, contact_flags, time_interval, time_steps)
    ...
    add_smoothness_cost(prog, decision_variables, time_interval, time_steps)
    ...
    if result.is_success():
        ...
        w_fl_opt, w_fr_opt, w_rl_opt, w_rr_opt = np.zeros((time_steps, robot.n_q)), np.zeros((time_steps, 3)), \
                                                 ...
                                                 np.zeros((time_steps, 3)), np.zeros((time_steps, 3)), \
                                                 np.zeros((time_steps, 1)), np.zeros((time_steps, 1)), \
                                                 np.zeros((time_steps, 1)), np.zeros((time_steps, 1))
