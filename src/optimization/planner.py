...
def add_decision_variables(prog, n_q, time_steps):
    ...
    r_fl = prog.NewContinuousVariables(rows=time_steps, cols=2, name='r_fl')
    r_fr = prog.NewContinuousVariables(rows=time_steps, cols=2, name='r_fr')
    r_rl = prog.NewContinuousVariables(rows=time_steps, cols=2, name='r_rl')
    r_rr = prog.NewContinuousVariables(rows=time_steps, cols=2, name='r_rr')
...
def set_state_constraints(prog, robot, decision_variables, time_steps):
    ...
    # Base bounds
    #q_b_lb = np.array([- np.Inf] * 6)
    #q_b_ub = np.array([np.Inf] * 6)
    q_b_lb = np.array([-10] * 6)
    q_b_ub = np.array([10] * 6)
...
def set_kinematics(prog, robot, decision_variables, time_steps):
    ...
    for t in range(1, time_steps):
        residuals = robot.kinematic_2D_residuals(q[t], x[t], r_fl[t], r_fr[t], r_rl[t], r_rr[t])
...
def add_kinematics_cost(prog, robot, decision_variables, time_steps):
    ...
    for t in range(1, time_steps - 1):
        ...
        # Add 2D cost
        cost = prog.AddCost(robot.kinematic_2D_cost_function, vars=z)
        cost.evaluator().set_description(f'Kinematic 2D cost iteration: {t}')

        # Add height cost
        #fl_fk, fr_fk, rl_fk, rr_fk = robot.forward_kinematics(q[t], 'sym')
        #prog.AddCost((fl_fk[2] - rz_fl[t]) * (fl_fk[2] - rz_fl[t]))
...
def add_smoothness_cost(prog, decision_variables, time_interval, time_steps):
    ...
    for t in range(1, time_steps - 1):
        ...
        prog.AddQuadraticCost(((q[t - 1] - 2 * q[t] + q[t + 1]) / time_interval**2).dot((q[t - 1] - 2 * q[t] + q[t + 1]) / time_interval**2))
...
def run_NLP(robot, q_start, weight_lb, omega_lb, omega_ub, desired_net_distance, desired_turning_angle, \
    ...
    if result.is_success():
        ...
        w_fl_opt, w_fr_opt, w_rl_opt, w_rr_opt = np.zeros((time_steps, robot.n_q)), np.zeros((time_steps, 3)), \
                                                 np.zeros((time_steps, 2)), np.zeros((time_steps, 2)), \
                                                 np.zeros((time_steps, 2)), np.zeros((time_steps, 2)), \
